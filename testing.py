...
print(f"угадано: {len(data.ids)/data.df.shape[0]}")
...
for el in ability[:-1]:
    print(f"For Class-{int(el[0])} \t compactness = {el[1]*100:3.2f}%")
print(f"General compactness = {ability[-1][1]*100:3.2f}%")
print(f"time: {time.time() - start_time:3.2f} secs\n")
print(f"\n-----------------------------------------\n")
...
print(f"Правильность на тестовом наборе, при n={n_neighbors}:\n"
      f"Обычный: {knn.score(X_test, y_test)*100:3.2f}%;\t", end=' ')
if len(etalon) > n_neighbors:
    if po_etalonu:
        print(f"NUU'кий: {nuu_knn.score(NX_test, Ny_test)*100:2.2f}%")
    else:
        print(f"NUU'кий: {nuu_knn.score(NX_test, Ny_test)*100:2.2f}%")
else:
    print(f"NUU'кий: ?")
